...
<!--    </Select>-->
    ...
    <Button style="float: right;" :to="{ name: 'department_add' }" type="primary">新增部门</Button>
  </div>
...
export default {
  ...
  methods: {
    getData () {
      const that = this
      that.$api.department.list(that.params).then(res => {
    ...
    remove (index, id) {
      this.$api.department.del({ id: id }).then(res => {
        if (res.data.errcode === 200) {
