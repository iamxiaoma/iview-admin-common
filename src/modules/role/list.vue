...
<!--    </Select>-->
    ...
    <Button style="float: right;" :to="{ name: 'role_add' }" type="primary">新增角色</Button>
  </div>
...
export default {
  ...
  methods: {
    getData () {
      const that = this
      that.$api.role.list(that.params).then(res => {
    ...
    show (index) {
      this.$Modal.info({
        title: '角色信息',
        content: `角色名称：${this.data[index].name}<br>角色简介：${this.data[index].intro}<br>角色状态：${this.data[index].status_name}`
      })
    },
    remove (index, id) {
      this.$api.role.del({ id: id }).then(res => {
        if (res.data.errcode === 200) {
