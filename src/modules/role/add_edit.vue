<template>
  <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
    ...
    <FormItem label="角色简介" prop="intro">
      <Input v-model="formValidate.intro" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入角色简介"></Input>
    </FormItem>
...
export default {
  ...
  data () {
    return {
      ...
      ruleValidate: {
        ...
        intro: [
          { required: true, message: '请填写角色简介', trigger: 'blur' }
        ],
        status: [
  ...
  methods: {
    detail () {
      this.$api.role.detail({ id: this.id }).then(res => {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.id == null) {
            this.$api.role.add(this.formValidate).then(res => {
